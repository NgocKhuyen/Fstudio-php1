<?php
    ...
    function product_hot() {
        $sql = "SELECT * FROM SanPham WHERE Hot = 1 ORDER BY MaSanPham DESC LIMIT 4";
    ...
    function product_page($page) {
        $start = ($page - 1) * 5;
        $sql = "SELECT sp.*, dm.TenDanhMuc FROM SanPham sp INNER JOIN DanhMuc dm ON sp.MaDanhMuc = dm.MaDanhMuc ORDER BY sp.MaSanPham DESC LIMIT $start, 5";
        return pdo_query($sql);
    }
    ...
    function product_add($name, $category, $price, $sale, $description, $image, $quantity, $hot, $status) {
        $sql = "INSERT INTO SanPham(TenSanPham, HinhAnh, Gia, GiaKhuyenMai, MaDanhMuc, SoLuong, MoTa, Hot, TrangThai)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        return pdo_execute($sql, $name, $image, $price, $sale, $category, $quantity, $description, $hot, $status);
    }
    ...
    function product_edit($id, $name, $category, $price, $sale, $description, $image, $quantity, $hot, $status) {
        $sql = "UPDATE SanPham SET TenSanPham = ?, HinhAnh = ?, Gia = ?, GiaKhuyenMai = ?, MaDanhMuc = ?, 
                        SoLuong = ?, MoTa = ?, Hot = ?, TrangThai = ?
                WHERE MaSanPham = ?";
